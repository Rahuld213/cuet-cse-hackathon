...
const DownloadInitiateRequestSchema = z.object({
  file_ids: z
...
const DownloadInitiateResponseSchema = z.object({
  jobId: z.string(),
...
const DownloadStartRequestSchema = z.object({
  file_id: z.number().int().min(10000).max(100000000),
});

const DownloadStartResponseSchema = z.object({
  file_id: z.number().int(),
...
// Async Download Start Route - returns immediately with job ID
const downloadStartRoute = createRoute({
  method: "post",
  path: "/v1/download/start",
  tags: ["Download"],
  summary: "Start file download (async)",
  description: `Starts a file download asynchronously. Returns immediately with a job ID.
    Use /v1/download/status/{jobId} to check progress.
  ...
  responses: {
    202: {
      description: "Download job started (async)",
      content: {
        "application/json": {
          schema: z.object({
            jobId: z.string(),
...
const downloadStatusRoute = createRoute({
  ...
  description: "Check the status of an async download job",
  request: {
    params: z.object({
      jobId: z.string(),
    }),
      ...
      content: {
        "application/json": {
          schema: z.object({
            ...
            result: DownloadStartResponseSchema.nullable(),
...
const processDownloadJob = async (jobId: string, fileId: number) => {
  ...
  try {
    ...
    console.log(
      `[Download] Processing job=${jobId} file_id=${String(fileId)} | delay=${delaySec}s (range: ${minDelaySec}s-${maxDelaySec}s) | enabled=${String(env.DOWNLOAD_DELAY_ENABLED)}`,
    );
    ...
    console.log(
      `[Download] Completed job=${jobId} file_id=${String(fileId)}, actual_time=${String(processingTimeMs)}ms, available=${String(s3Result.available)}`,
    );

    // Update job with result
    job.endTime = Date.now();
    job.status = s3Result.available ? "completed" : "failed";
...
app.openapi(downloadStartRoute, (c) => {
  const { file_id } = c.req.valid("json");
  const jobId = crypto.randomUUID();
  ...
  processDownloadJob(jobId, file_id).catch(console.error);
  ...
  return c.json(
    {
      ...
      processingTimeMs: job.endTime ? job.endTime - job.startTime : null,
